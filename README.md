...
1. 설정 패널(⚙️)에서 "로그 관리" 섹션 확인
2. 로그 통계 확인 (전체, 크기, DEBUG/INFO/WARN/ERROR 개수)
3. 로그 수집 활성화/비활성화 토글
4. "JSON 다운로드" 또는 "TXT 다운로드" 버튼으로 로그 파일 저장
5. "로그 삭제" 버튼으로 모든 로그 초기화

#### 로그 기능 설명

- **자동 저장**: 모든 로그는 localStorage에 자동 저장 (최대 1000개)
- **로그 레벨**: DEBUG, INFO, WARN, ERROR 4단계 지원
- **카테고리**: WebSocket, Ping, Terminal, Connection, Store 등으로 분류
- **파일 형식**: JSON (구조화된 데이터) 또는 TXT (가독성 높은 텍스트)
