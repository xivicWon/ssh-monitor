...
- 출력 버퍼 크기: 8KB
- JSch ChannelShell 사용 (Interactive Shell)
- UTF-8 인코딩 사용
...
public interface SshConnectionService {
    ...
    /**
     * JSch Session 생성 (내부 헬퍼)
     */
    Session createSession(SshConnectionRequest request) throws JSchException;

    /**
     * SSH 명령 실행 (내부 헬퍼)
     */
    String executeCommand(Session session, String command, int timeoutSeconds) throws Exception;
}
```

**구현 포인트:**
- JSch 라이브러리 사용
- StrictHostKeyChecking: no (MVP에서는 검증 생략)
- 연결 타임아웃: 10초
- 명령 실행 타임아웃: 5초
- 예외 처리: JSchException, IOException 등을 catch하여 적절한 에러 코드로 변환
...
private static class TerminalSession {
    ...
    private final Session sshSession;      // JSch Session
    private final ChannelShell channel;    // JSch ChannelShell
...
#### 5.1 환경 설정
- [ ] Spring Boot 프로젝트 생성 (Java 21, Spring Boot 3.2+)
- [ ] JSch 의존성 추가 (`com.jcraft:jsch:0.1.55`)
...
2. **리소스 관리**
   - 세션 종료 시 모든 리소스 정리 (Session, Channel, Stream, Thread)
...
1. **기능 제한**
   - 동시 세션: 최대 10개
   - SSH 키 인증: PEM 포맷만 지원
   - StrictHostKeyChecking: 비활성화 (known_hosts 검증 생략)
...
channel.setPtySize(cols, rows, 640, 480);
